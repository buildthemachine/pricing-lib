...
class test_american_put_carr(unittest.TestCase):
    ...
    def test_price(self):
        ...
        prices = {}
        s_low, s_high = 30, 200
        spots = np.linspace(s_low, s_high, 1000)
        ...
        for order in range(2, 5, 2):
            prices[order] = []
            for x0 in spots:
                self.american_opt_call_randomization = (
                ...
                prices[order].append(self.american_opt_call_randomization.Price())
            plt.plot(spots, prices[order], label=f"order={order}")
        euro_price = []
        for x0 in spots:
            # Analytical solution for Black-Scholes
            self.bs_call = instruments.european_option(
                isCall=True,
                strike=self.strike,
                underlying=x0,
                expiration=self.tau,
            ...
            euro_price.append(self.bs_call.Price())
        plt.plot(spots, euro_price, label="European")
        plt.plot(spots, np.maximum(spots - self.strike, 0), label="payoff")
        plt.xlim((s_low, s_high))
        plt.ylim((0, s_high - self.strike))
        plt.legend(loc="upper left")
        plt.show()

        # Plot put options:
        fig = plt.figure(figsize=(15, 10))
        for order in range(2, 3, 2):
            prices[order] = []
            for x0 in spots:
